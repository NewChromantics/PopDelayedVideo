Shader "NewChromantics/FishEyeDistortion And Shrink"
{
	Properties
	{
		...
		ShrinkCenter_U("ShrinkCenter_U", Range(0,1) ) = 0.5
		ShrinkCenter_V("ShrinkCenter_V", Range(0,1) ) = 0.5
		ShrinkScale("ShrinkScale", Range(0,1) ) = 1
		ShrinkClipColour("ShrinkClipColour", COLOR ) = (0,0,0,1)
	}
	SubShader
	{
		...
		Pass
		{
			...
			float ShrinkCenter_U;
			float ShrinkCenter_V;
			float ShrinkScale;
			float4 ShrinkClipColour;

			float2 ShrinkUv(float2 uv,out bool Clip)
			{
				uv -= float2( ShrinkCenter_U, ShrinkCenter_V );
				uv /= ShrinkScale;
				uv += float2( ShrinkCenter_U, ShrinkCenter_V );

				Clip = ( uv.x < 0 ) || ( uv.x > 1 ) || ( uv.y < 0 ) || ( uv.y > 1 );
